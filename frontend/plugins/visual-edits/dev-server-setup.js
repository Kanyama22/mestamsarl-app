...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow only localhost/127.0.0.1 and appspot subdomains
      if (origin.match(/^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="visual-edit@example.com" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="visual-edit@example.com" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
